            ...
            {items.map((item) => (
              <Accordion.Item key={item.question} value={item.question} className="border-b border-border">
                <Accordion.Header render={<h3 />}>
                  <Accordion.Trigger className="group flex w-full items-center justify-between gap-6 py-5 text-left text-[15px] font-semibold text-surface-dark transition-colors hover:text-accent-foreground-strong">
                    {item.question}
                    <ChevronDown className="size-5 shrink-0 text-accent-foreground-strong transition-transform duration-300 group-data-[panel-open]:rotate-180" aria-hidden="true" />
                  </Accordion.Trigger>
                </Accordion.Header>
                <Accordion.Panel className="h-[var(--accordion-panel-height)] overflow-hidden transition-[height] duration-300 ease-out data-[starting-style]:h-0 data-[ending-style]:h-0">
                  <p className="max-w-xl pb-8 text-sm leading-relaxed text-muted-foreground">{item.answer}</p>
                </Accordion.Panel>
              </Accordion.Item>
            ))}
          </Accordion.Root>
